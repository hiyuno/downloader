...
| 2026-07-27 | `formatArgument` de `DownloadQuality` filtra por `vcodec^=avc1` antes que por `[ext=mp4]` | Bug reportado por el usuario: videos de Instagram descargaban como `.mp4` pero no abrían en QuickTime. Causa: Instagram sirve algunos streams VP9 dentro de un contenedor rotulado `.mp4`; `--merge-output-format mp4` los remuxea (no transcodifica) a un `.mp4` técnicamente válido pero con video VP9 adentro, que QuickTime/AVFoundation no puede decodificar en ningún contenedor. `[ext=mp4]` solo mira la extensión del contenedor, no el códec — no era suficiente. Fix: cada nivel de calidad ahora prioriza `vcodec^=avc1` (H.264, siempre reproducible en QuickTime) y solo cae al viejo filtro `[ext=mp4]`/`b` genérico si el sitio de verdad no ofrece ninguna variante H.264 |
