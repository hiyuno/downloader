...
| Espacio entre la superficie principal y el chip de advertencia | 6pt |
...
**Ancho fijo:** 560pt, sin cambios. La superficie principal mide **siempre 64pt** (6pt de padding + frame de 52pt + 6pt de padding) y nunca cambia de tamaño. La única variación del `NSPanel` es el espacio transparente necesario para mostrar el chip flotante debajo, solo en `.input`:

| Estado del panel | Alto |
|---|---|
| Frame en `.input`, `.downloading`, `.completed` o `.error` (sin chip) | 64pt (6 padding + 52 frame + 6 padding) |
| Frame en `.input` con chip de advertencia visible | 92pt (64 superficie + 6 separación + 22 chip) |

El ancho **nunca** cambia. El alto ahora tiene solo dos valores posibles en toda la app (64pt / 92pt) en vez de una escala continua hasta 420pt — la lista, el `ScrollView` interno y el techo `panelMaxHeight` quedan retirados junto con `DownloadRowView` como componente de lista (ver Decisiones registradas).

**Layout vertical (de arriba hacia abajo):**
1. Superficie principal de vidrio (64pt) con el frame de 52pt dentro — su contenido cambia según el estado, pero la superficie nunca crece.
2. Chip flotante (solo si hay una advertencia en `.input`) — 22pt, separado 6pt y alineado con el inicio del texto del input. Vive fuera del fondo de vidrio principal para que la advertencia se lea como información auxiliar, no como parte del control.
...
| `.cancelled` | "Cancelado" |
| `.liveStream` | "This is a live stream — it can't be downloaded until it ends" (**en inglés, no traducido** — el resto de esta tabla documenta el copy en español por convención histórica del documento, pero el string real en código ya está en inglés como todo el texto nuevo de la UI; ver "Decisiones registradas" 2026-08-23) |
...
| 2026-07-27 | Padding interno del panel reducido a la mitad: 12pt → 6pt (`Theme.Spacing.panelPadding`) | Pedido directo del usuario — el aro de glass visible alrededor del frame se sentía demasiado ancho. Cascada obligatoria por la regla `r_inner = r_outer − padding`: el radio del frame sube de 12pt a **18pt** (`Theme.Radius.row`, sigue siendo 24 − 6), y la superficie principal baja de 76pt a **64pt**. El radio de reserva del tile de ícono de sitio (hoy sin uso, ver sección 2) se actualiza en paralelo a 10pt (18 − 8) para no quedar inconsistente si se reintroduce |
| 2026-08-11 | Chip de advertencia movido fuera de la superficie principal | La cápsula es información auxiliar y no debe agrandar el bloque de vidrio principal. Flota 6pt debajo, alineada con el texto del input; el `NSPanel` mide 92pt mientras está visible. |
...
| 2026-08-23 | Detección de streams en vivo: la app avisa en vez de intentar una descarga infinita | Reporte del usuario: un directo de YouTube descargaba sin fin. Un directo HLS no tiene tamaño fijo — la descarga nunca terminaba sola, y si la calidad configurada no existía en los formatos del directo, yt-dlp fallaba con "Requested format is not available" que la app mostraba como "El sitio cambió o bloqueó la descarga", un mensaje que confundía sobre la causa real. Se agrega `DownloadFailureReason.liveStream` (caso nuevo, no reutiliza `.siteBlockedOrChanged`) con mensaje propio; reusa el estado `.error` del frame existente sin cambios de spec — no hizo falta un quinto estado. Detección: se pide `%(live_status)s` en el mismo proceso de yt-dlp que ya corre para el título/ruta (`--print video:[live]%(live_status)s`, marcador nuevo en el mismo mecanismo de `[dl]`/`[title]`/`[file]`), sin lanzar una consulta de metadatos aparte — el stage `video` de yt-dlp resuelve justo después de `-f` y antes de `before_dl`, así que se detecta y se mata el proceso (yt-dlp + hijos, reusando `terminateChildren`) antes de bajar un solo byte. Solo `is_live`/`is_upcoming` bloquean; `was_live` (directo ya terminado, ahora VOD) se descarga normal — verificado contra streams reales: dos noticieros 24/7 de YouTube actualmente en vivo (ABC News Live, France 24 English) resolvieron `is_live`; dos videos normales (Rick Astley, "Me at the zoo") resolvieron `not_live`; tres replays de lanzamientos de SpaceX Starship ya terminados resolvieron `was_live` |
